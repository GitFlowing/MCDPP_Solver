	...
	const LambdaZustand& lambda_option, const VerzweigungZustand& verzweigung_option)
{
	...
	// l_upper muss gr��er sein, als obere_schranke
	...
	{
		throw invalid_argument("Kantengewichte des Basisgrpahen zu gro�. Branch and Bound nicht anwendbar!");
	}

	// Counter f�r Anzahl an Iterationen des B&B
	size_t counter = 0;


	// F�r Subgradientenverfahren Knoten Grad zuordnen
	...
	// Anfangsinstanzen f�r Warteschlange erzeugen
	...
	// Unzul�ssige oder suboptimale L�sung am Anfang -> es gibt keine L�sung
	...
	// Optimale L�sung am Anfang -> Ende des B&B
	...
	// Start Instanz zur Warteschlange hinzuf�gen (muss Kandidat sein)
	warteschlangeBB.push(instanz_auswahl.verzweigte_subinstanz[0]);


	// Gr��en f�r Loop initialisieren
	...
	// Solange Warteschlange noch nicht leer ist wird B&B ausgef�hrt
	while (!warteschlangeBB.empty())
	{
		// Subinstanz mit kleinster unterer Schranke zum Verzweigen ausw�hlen
		...
		// Subinstanzen zur Warteschlange hinzuf�gen
		...
		// Wenn sich kleinste obere Schranke �ndert -> Ausgabe der zul�ssigen L�sung
		...
		// Update obere Schranke f�r die Ausgabe
		obere_schranke_kopie = warteschlangeBB.getObereSchranke();

		counter++;
	}

	// Endergebnis initialisieren, wenn es eine optimale L�sung besitzt
...
void instanzTerminalsInitialisieren(const Graph& base_graph, const Graph& demand_graph, BB_Instanz& instanz_anfang)
{
	...
	// Anfangswerte f�r lambda f�r das Subgradientenverfahren festlegen
	...
	const VerzweigungZustand& verzweigung_option)
{
	...
	// Suchen den Knoten, der die kleinste untere Schranke nach Verzweigung maximal erh�ht
	double max_untere_schranke = (-1) * UNENDLICH;

	// Gr��en f�r Loop, zum Suchen der kleinsten unteren Schranke nach Verzweigung
	double kleinster_weg_dist = UNENDLICH;
	size_t knoten_kleinste_dist = base_graph.anzKnoten();

	// Alle Kandidaten, die als Verzweigungsknoten m�glich sind
	...
	// -> Verzweigung durchf�hren
	// -> kleinste untere Schranke nach Verzweigung bilden
	// -> Speicher den Knoten, der nach Verzweigung gr��ten Wert der kleinsten untere Schranke hat
	for (size_t knoten : weg_knoten)
	{
		...
		// Speichern der zul�ssigen L�sung mit geringsten Gesamtkosten
		// Update globale obere Schranke mit zul�ssiger L�sung
		...
		// Update Knoten mit gr��ten Wert der kleinsten unteren Schranke nach Verzweigung
	...
	// Sollte kein Knoten gefunden werden, einfach letzten Knoten w�hlen
...
// Kandidaten f�r Verwzeigungsknoten
vector<size_t> verzweigungsKnotenFinden(const Graph& base_graph, const BB_Instanz& neue_instanz, double l_upper, const VerzweigungZustand& verzweigung_option)
{
	...
	// Wegknoten, Verzweigungsknoten und Flussz�hler in jeden Knoten
	unordered_set<size_t> weg_knoten;
	vector<size_t> weg_knoten_counter(base_graph.anzKnoten(), 0);
	vector<size_t> verzweigungs_knoten;

	// Knoten mit Weg maximaler L�nge und Knoten mit max. Fluss speichern
	...
	for (const pair<size_t, size_t>& terminal : neue_instanz.alle_demands)
	{
		...
		// Terminals nicht betretbar, au�er Start- und Endterminal
		...
		// L�ngsten Dijkstra-Weg ermitteln -> davon mittleren Knoten -> mindestens 3 Knoten im Weg
			...
			&& gefundener_weg.weg_knoten.size() > 2)
		{
			...
			// Von dem l�ngsten Weg mittleren Knoten speichern
		...
		//Alle gel�schten Terminal-Knoten wieder zur�cksetzen
	...
	{
		//Mittlerer Knoten des l�ngsten Weges des eingebetteten Demands
	...
	{
		// Nur mehrfach durchflossene w�hlbare Wegknoten als Kandidaten f�r Verzweigungsknoten w�hlen
		size_t anzahl = 0;
		for (size_t knoten : weg_knoten)
		{
			anzahl++;
		...
		// Spezialfall zul�ssige L�sung -> keine Konflikte
...
{
	// Es muss mindestens ein weiterer nicht gel�schter Nicht-Terminalknoten existieren,
	// sonst w�re keine Verzweigung m�glich gewesen
	unordered_set<size_t> alle_nicht_waehlbaren_knoten;

	// Alle Terminals nicht w�hlbar
	...
	// Alle gel�schten Knoten nicht w�hlbar
	for (size_t element : instanz_auswahl.geloeschte_knoten)
	{
		alle_nicht_waehlbaren_knoten.insert(element);
	}

	// W�hle ersten Knoten aus Basis-Graphen der nicht in alle_nicht_waehlbaren_knoten ist
	...
	double obere_schranke, const vector<size_t>& terminals_grad, size_t max_iter_sub, size_t n_iter_half, double l_upper, const LambdaZustand& lambda_option)
{
	...
	// Verzweigungsknoten als Zwischenterminal jedes Demands einf�gen oder  Verzweigungsknoten l�schen
	...
	for (int i = 0; i <= instanz_auswahl.alle_demands.size(); i++)
	{
		...
		// Untere Schranke = obere Schranke -> optimale L�sung der Subinstanz -> ist nicht Kandidat
	...
	size_t max_iter, double obere_schranke, size_t n_iter_half, double l_upper, const LambdaZustand& lambda_option)
{
	...
	// Verzweigungsknoten in Subinstanz einf�gen
	if (demand_index == -1)
	{
	...
	{
		// Verzweigungsknoten wird gel�scht
...
vector<vector<size_t>> kantenAusgabe(const BranchBoundErgebnis& ergebnis, const Graph& base_graph, const Graph& demand_graph)
{
	...
	// Alle Weg Kanten der demands der L�sung durchgehen
	// Terminalkanten identifizieren -> Start des Weges f�r den Demand
	// Knoten der Terminalkante zur n�chster Wegkante identifizieren und n�chste Kante finden
	// gelangt man wieder zu einer Terminalkante -> Ende des Weges

	for (size_t kante : alle_demand_weg_kanten)
	{
		// Ist erste Kante Terminal-Kante und noch f�r keinen Weg verwendet worden, so beginnt dort der Weg
			...
			&& schon_gesetzte_wegkanten.find(kante) == schon_gesetzte_wegkanten.end())
		{
			...
			// Terminalkante als Start hinzuf�gen zum Weg
			weg.push_back(kante);
			schon_gesetzte_wegkanten.insert(kante);

			// Fussknoten, Kopfknoten der Kante ermitteln -> Anfangsknoten der n�chsten Kante (wahl_knoten)
				...
				|| terminal_knoten.find(kopf_knoten) == terminal_knoten.end())
			{
				...
				FUER_ALLE_NACHBARN(n, base_graph, wahl_knoten)
				{
					...
					{
						// N�chste Kante des Weges bestimmen und hinzuf�gen
						// N�chster Knoten ist automatisch Nachbarknoten (wahl_knoten)
						naechste_kante = n.j();
						wahl_knoten = n.i();
				...
				// Solange n�chste Kante keine Terminalkante ist, endet der Weg nicht
			...
			// Weg wurde aus Kanten erstellt -> eingebetteter Weg zu erstellte_wege hinzuf�gen
			erstellte_wege.push_back(weg);
		}
	}


	// Wege in die Reihenfolge der Demands im Demand-Graphen bringen -> jedem Demand l�sst sich eindeutig Weg zuordnen
	vector<vector<size_t>> wege_reihenfolge;
	...
	FUER_ALLE_IKANTEN(j, demand_graph)
	{
		...
		for (size_t w = 0; w < erstellte_wege.size(); w++)
		{
			...
			// Fu�- und Kopfknoten von erster und letzter Kante in Menge vereinigen
			alle_knoten.clear();
			...
			// -> das ist der Weg f�r diesen demand
